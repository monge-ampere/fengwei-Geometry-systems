...
convergRadius=...
convergRadius4Break=...
convergRadius4ColorDiff=...
convergRadius4DiaLarge=...

[gParam]
availNeighbourRate4Break=...
availNeighbourRate4ColorDiff=...
breakPointNumThres=...
...
bgRate=...
```
如果只看这些名字，确实会觉得参数不少。但稍微往下看一层，就会发现，它们并不是杂乱堆在一起的。

一类参数，直接对应局部测量对象本身。比如 `lightThres`、`darkThres` 对应灰度与对比关系，`diaThres`、`maxDiaThres` 对应局部宽度或直径类量，`detNormThres` 则更接近整体失配或异常程度的辅助量。也就是说，这些参数并不是随意拍脑袋写出来的，而是挂在“系统到底在测什么”这一层上的。

另一类参数，对应的是点级异常如何上升为最终缺陷。比如 `breakPointNumThres`、`colorDiffPointNumThres`、`ngPointNumThres`、`largeDiaPointNumThres` 这些量，控制的并不是单点测量本身，而是异常点在邻域内达到什么规模、什么密度之后，才足以形成可输出的缺陷。这也解释了为什么系统不会因为某一个孤立点的轻微异常就立刻乱报。

还有一类参数，对应的是局部展开方式和测量环境本身。比如 `availNeighbourRate4Break`、`availNeighbourRate4ColorDiff`、`binaryzationType`、`bgRate` 等，控制的是局部邻域中哪些信息被视为有效、局部投影怎样形成、背景与结构如何区分。它们本质上仍然是在服务“沿测量点展开局部测量”这一条主线。
