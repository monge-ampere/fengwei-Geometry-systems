...
模板创建工具里有一个很值得注意的细节：在打开原始灰度图像之后，界面上优先显示的并不是原图本身，而是算法处理后的二值图像。文档里对这一点说得很明确：显示二值图像的目的，是为了能够清晰查看并确保模板是有效的。

这一点表面上看只是界面选择，但它其实非常能说明问题。

因为这意味着模板创建并不是站在“人眼看着像不像”的层面上做判断，而是站在后续检测链的视角上做判断。系统真正关心的，并不是原图在视觉上是否漂亮，而是经过处理之后，这个局部区域还能不能稳定地支撑骨架抽取、中心点生成和后续配准测量。

换句话说，模板创建从一开始就已经不是视觉截图，而是在做“算法视角下的结构确认”。
